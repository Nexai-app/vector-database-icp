...
describe("access control should work", async () => {
  ...
  it("should not allow unprivileged user add manager or add allow list", async () => {
    expect(
      async () => await vdb_user.add_manager(user.getPrincipal())
    ).rejects.toThrowError("Not owner");
    expect(
      async () => await vdb_user.add_accesser(user.getPrincipal())
    ).rejects.toThrowError("Not manager");
  });

  it("should not allow manager to add another manager", async () => {
    expect(
      async () => await vdb_manager.add_manager(manager.getPrincipal())
    ).rejects.toThrowError("Not owner");
  });
  ...
  it("should allow user to register and manage their own vdb", async () => {
    let res = (await vdb_user.register("Description of the company")) as {
      Ok: number;
    };
  ...
  it("should allow user to insert keys and values", async () => {
    ...
    let values = ["aaa", "bbb"];
    let append_res = (await vdb_user.append_keys_values(
      comp_id,
      keys,
      values
    )) as { Ok: null };
  ...
  it("should allow anyone to register", async () => {
    ...
    let res = (await vdb_anyone.register("anyone can register")) as {
      Ok: number;
    };
    expect("Ok" in res).eq(true);
  });
...
describe("vector database should work", async () => {
  ...
  c[0] = 0;
  let embds = [a, b, c];
  let values = ["question 1?", "question 4?", "question 6?"];
  ...
  it("should return a same value corresponding to embedding", async () => {
    // embedding, limit
    // TODO: Check this
    let result = await vdb.get_similar(company_id, "a", [1], 1);
    expect("Ok" in result).eq(true);
    if ("Ok" in result) {
      const vs = result.Ok;
